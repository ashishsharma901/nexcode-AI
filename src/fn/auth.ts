...
// These are server functions — they run on the server but are callable from the client via RPC.
// They must NOT be in src/server/ directory (which is blocked from client imports).
// The actual server-only logic (db, bcrypt, jwt) is imported inside the handler closures.
...
export const registerUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { createAccessToken, createRefreshToken } = await import("@/server/auth/jwt");
    type TokenPayload = import("@/server/auth/jwt").TokenPayload;
...
export const refreshAccessToken = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { db } = await import("@/server/db");
    const { verifyToken, createAccessToken } = await import("@/server/auth/jwt");
